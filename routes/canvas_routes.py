...
- You may receive an ANALYSIS DOCUMENT below containing pre-analyzed insights from transcripts
- This document was generated by a specialized analysis agent with full system context
- Use it as your primary knowledge source for the current mode (mirror/lens/portal)
- If no analysis document is present, respond based on the user's immediate message only
- Do not mention the analysis document explicitly to the user
    ...
    def handle_canvas_stream(user: SupabaseUser):
        ...
        def generate_canvas_stream():
            ...
            try:
                ...
                # Get or generate analysis document for this mode
                analysis_doc = None
                try:
                    analysis_doc = get_or_generate_analysis_doc(
                    ...
                    if analysis_doc:
                        logger.info(f"Canvas: Loaded analysis document for {depth_mode} mode ({len(analysis_doc)} chars)")
                    else:
                        logger.info(f"Canvas: No analysis document available for {depth_mode} mode")
                except Exception as analysis_err:
                    logger.error(f"Error getting analysis document: {analysis_err}", exc_info=True)
                    analysis_doc = None
                ...
                # Combine: Canvas Base + Depth + Analysis Doc + Agent + Time
                system_prompt = canvas_base_and_depth

                # Insert analysis document if available
                if analysis_doc:
                    system_prompt += f"\n\n=== ANALYSIS DOCUMENT ===\n{analysis_doc}\n=== END ANALYSIS DOCUMENT ==="
                ...
                prompt_type = "base+depth+analysis+agent+time" if analysis_doc else "base+depth+agent+time"
