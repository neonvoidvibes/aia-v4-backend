...
def maybe_trim_repetition(
    ...
    tail = state.last_tokens[-_MAX_CTX_TOKENS:]
    head = new_tokens[:_MAX_CTX_TOKENS]
    cut_idx = 0

    if tail and head:
        l = _lcs_len(tail, head)
        if l >= _MIN_OVERLAP_TOKENS:
            ...
            if cut_idx >= _MIN_OVERLAP_TOKENS:
    ...
    if new_tokens:
        ...
        if head_tuple in state.recent_heads:
        ...
        # Update head cache AFTER potential drop
        if new_tokens:
            state.recent_heads.append(tuple(new_tokens[:_HEAD_DEDUP_N]))

    if not new_tokens:
        logger.info("segment completely trimmed - nothing new after overlap/dedup")
        return "", "completely_trimmed", -1.0

    # Calculate cut boundary for word-level trimming
    cut_s = -1.0
    if len(original_tokens) > len(new_tokens) and curr_words:
    ...
    reason = "overlap_trimmed" if cut_idx > 0 or len(original_tokens) > len(new_tokens) else ""
